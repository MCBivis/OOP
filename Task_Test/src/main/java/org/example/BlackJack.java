...
public class BlackJack {
  ...
  private void playerTurn(Deck deck, Hand playerHand, Hand dealerHand) {
    ...
    while (choice != 0) {
      ...
      if (choice == 1) {
        playerHand.add(deck.draw());
        System.out.println(
            "Вы открыли карту " + playerHand.cards[playerHand.InHand - 1].CardShow());
        System.out.println("\tВаши карты: " + playerHand.HandShow() + " => " + playerHand.score);
        System.out.println("\tКарты дилера: " + dealerHand.HandShow(0) + "\n");
      }
  ...
  private void dealerTurn(Deck deck, Hand playerHand, Hand dealerHand) {
    ...
    System.out.println(
        "Дилер открывает закрытую карту " + dealerHand.cards[dealerHand.InHand - 1].CardShow());
    System.out.println("\tВаши карты: " + playerHand.HandShow() + " => " + playerHand.score);
    System.out.println(
        "\tКарты дилера: " + dealerHand.HandShow() + " => " + dealerHand.score + "\n");

    // Дилер берет карты, пока его счет меньше 17
    while (dealerHand.score < 17) {
      dealerHand.add(deck.draw());
      System.out.println(
          "Дилер открывает карту " + dealerHand.cards[dealerHand.InHand - 1].CardShow());
      System.out.println("\tВаши карты: " + playerHand.HandShow() + " => " + playerHand.score);
      System.out.println(
          "\tКарты дилера: " + dealerHand.HandShow() + " => " + dealerHand.score + "\n");
    }
  ...
  private void win() {
    this.playerScore++;
    System.out.println(
        "Вы выиграли раунд! Счет " + this.playerScore + ":" + this.dealerScore + "\n");
  }
  ...
  private void lose() {
    this.dealerScore++;
    System.out.println(
        "Вы проиграли раунд! Счет " + this.playerScore + ":" + this.dealerScore + "\n");
  }
